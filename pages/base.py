"""Этот модуль содержит в себе класс BasePage, который является базовым для всех страниц. 
Для описания новой страницы, необходимо отнаследоваться от класса BasePage  и внутри 
__init__ мутода новой страницы вызвать __init__ метод
этого класса, передв в него URL желаемой страницы.
Класс имплементирует ряд базовых методов - navigate_to_url, scroll_up, scroll_down, go_back, get_current_url и др.
"""
...
class BasePage(ABC):
    ...
    def wait_for_page_reload(self):
        """Этот метод позволяет дождаться перезагрузки страницы. 
        Он определяет перезагрузку по появлению элемента body."""
        delay = WebDriverWait(self.driver, 10)
        return delay.until(expected_conditions.presence_of_element_located((By.TAG_NAME, "body")))

    def get_title(self):
        """Этот метод возвращает содержимое элемента title в блоке head страницы."""
